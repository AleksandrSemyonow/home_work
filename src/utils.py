...
# Создание обработчика для записи логов в файле
...
# Добавления обработчика к логгеру
...
def load_transaction(file_path):
    ...
    if not os.path.isfile(file_path):
        ...
        return []  # Если не существует, вернуть пустой список
    with open(file_path, "r", encoding="utf-8") as file:
        try:
            data = json.load(file)  # Попытка загрузить данный из файла
            if isinstance(data, list):  # Проверка, является ли загруженные данные списком
                logger.info(f"Транзакции успешно загруженны из файла: {file_path}")
                return data
            else:
